...
def save_images(data, output, target, epoch, testflag):
    ...
    save_folder = Path("/home/crcvreu.student10/run/sem_kfold/masks/")
...
def kfold_cross_validation(k, dataset, model_class, device, criterion, optimizer_class, epochs, batch_size, file, class_weights):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(dataset)):
        ...
        for epoch in range(1, epochs + 1):
            print(f'Epoch {epoch}')
            file.write(f'\nEpoch {epoch}\n')
            train_loss, train_acc, train_f1_class, train_f1 = train(model, device, train_loader, optimizer, criterion, epoch, batch_size, file, class_weights)
            val_loss, val_acc, val_f1_class, val_f1 = test(model, device, val_loader, file, epoch, class_weights)

            if val_acc > best_accuracy:
                best_accuracy = val_acc
                torch.save(model.state_dict(), f'checkpoint_fold_{fold+1}.pth')

        fold_results.append({
            'train_loss': train_loss,
            'train_acc': train_acc,
            'train_f1_class': train_f1_class,
            'train_f1': train_f1,
            'val_loss': val_loss,
            'val_acc': val_acc,
            'val_f1_class': val_f1_class,
            'val_f1': val_f1
        })

    # Average results across all folds
    avg_train_loss = np.mean([result['train_loss'] for result in fold_results])
    avg_train_acc = np.mean([result['train_acc'] for result in fold_results])
    avg_train_f1_class = np.mean([result['train_f1_class'] for result in fold_results], axis=0)
    ...
    avg_val_f1_class = np.mean([result['val_f1_class'] for result in fold_results], axis=0)
    ...
    file.write('Average Train Accuracy: {:.4f}\n'.format(avg_train_acc))
    file.write('Average Train F1 Score: {}\n'.format(avg_train_f1))
    file.write('Average Validation Loss: {:.4f}\n'.format(avg_val_loss))
    file.write('Average Validation Accuracy: {:.4f}\n'.format(avg_val_acc))
    file.write('Average Validation F1 Score: {}\n'.format(avg_val_f1))
...
def evaluation(f):
    ...
    model = torch.load('/home/crcvreu.student10/run/sem_kfold/final_model_filled2.pth')
    model.eval()
    ...
    img_dir = Path('/home/crcvreu.student10/run/sem_kfold/test/data/')
    mask_dir = Path("/home/crcvreu.student10/run/sem_kfold/test/output/")
    target_mask_dir = Path("/home/crcvreu.student10/run/sem_kfold/masks/target/")
    final_mask_dir = Path("/home/crcvreu.student10/run/sem_kfold/masks/final/")
    ...
    for file_path in img_dir.iterdir():
        if file_path.suffix in {'.png', '.tif', '.jpg'}:
            ...
            f1_weighted = f1_score(m.transform(label), m.transform(pred), average='weighted')
            f1_all.append(f1_weighted)
            if len(f1) != 6:
                continue
...
#     )
#     model = UNet_PV.to(device)
#     save_folder = "/home/crcvreu.student10/run/sem_kfold/masks"
#     checkpoint_folder = "/home/crcvreu.student10/run/sem_kfold/checkpoints"
#     optimizer_class = lambda params: optim.Adam(params, lr=FLAGS.learning_rate)
...
#     #################### LOAD DATA ####################
#     image_dir = '/home/crcvreu.student10/run/sem_kfold/data/'
#     label_dir = '/home/crcvreu.student10/run/sem_kfold/output'
#     train_image_dir = '/home/crcvreu.student10/run/sem_kfold/train/data/'
#     train_label_dir = '/home/crcvreu.student10/run/sem_kfold/train/output/'
#     test_image_dir = '/home/crcvreu.student10/run/sem_kfold/test/data/'
#     test_label_dir = '/home/crcvreu.student10/run/sem_kfold/test/output/'

#     # Split data into training and test sets
#     # split_data(image_dir, label_dir, train_image_dir, train_label_dir, test_image_dir, test_label_dir)
#     dataset = PVDataset(image_dir, label_dir, transform=transform)
...
#     kfold_cross_validation(6, dataset, UNet_PV, device, criterion, optimizer_class, FLAGS.epochs, FLAGS.batch_size, file, class_weights)


#     best_accuracy = 0.0
#     checkpoint_path = '/home/crcvreu.student10/run/sem_kfold/checkpoints/model_filled2.pth'
...
#     torch.save(model, '/home/crcvreu.student10/run/sem_kfold/final_model_filled2.pth')
...
#     plt.grid(True)
#     plt.ylim(0,1)
#     plt.savefig("/home/crcvreu.student10/run/sem_kfold/graphs/Train_Test_loss_model.jpg")
#     plt.clf()
...
#     plt.grid(True)
#     plt.ylim(0,1)
#     plt.savefig("/home/crcvreu.student10/run/sem_kfold/graphs/Train_Test_acc_model.jpg")
#     plt.clf()
...
#     plt.grid(True)
#     plt.ylim(0,1)
#     plt.savefig("/home/crcvreu.student10/run/sem_kfold/graphs/Train_f1_per_class_model.jpg")
#     plt.clf()
...
#     plt.grid(True)
#     plt.ylim(0,1)
#     plt.savefig("/home/crcvreu.student10/run/sem_kfold/graphs/Test_f1_per_class_model.jpg")
#     plt.clf()
...
#     plt.grid(True)
#     plt.ylim(0,1)
#     plt.savefig("/home/crcvreu.student10/run/sem_kfold/graphs/Train_Test_f1_model.jpg")
#     plt.clf()
#     file.write("\nTraining finished")
...
def plot_and_save_graphs(train_metrics, val_metrics, metric_name, fold, checkpoint_folder):
    epochs = range(1, len(train_metrics) + 1)
    plt.figure()
    plt.plot(epochs, train_metrics, 'b', label='Training ' + metric_name)
    plt.plot(epochs, val_metrics, 'r', label='Validation ' + metric_name)
    plt.title(f'Training and Validation {metric_name} - Fold {fold}')
    plt.xlabel('Epochs')
    plt.ylabel(metric_name)
    plt.legend()
    plt.savefig(checkpoint_folder / f'{metric_name.lower()}_fold_{fold}.png')
    plt.close()
...
def run_main(FLAGS, file):
    ...
    #################### LOAD DATA ####################
    # Directories
    image_dir = Path('/home/crcvreu.student10/run/sem_kfold/data/')
    label_dir = Path('/home/crcvreu.student10/run/sem_kfold/labels/')
    train_image_dir = Path('/home/crcvreu.student10/run/sem_kfold/train/data/')
    train_label_dir = Path('/home/crcvreu.student10/run/sem_kfold/train/output/')
    test_image_dir = Path('/home/crcvreu.student10/run/sem_kfold/test/data/')
    test_label_dir = Path('/home/crcvreu.student10/run/sem_kfold/test/output/')
    checkpoint_folder = Path("/home/crcvreu.student10/run/sem_kfold/checkpoints")
    ...
    if len(common_bases) == 0:
        raise ValueError("No common images and labels found for k-fold cross-validation.")
    
    images = [f"{base}.jpg" if f"{base}.jpg" in images else f"{base}.png" for base in common_bases]
    labels = [f"{base}_label.npy" for base in common_bases]

    # K-Fold Cross-Validation
    kf = KFold(n_splits=5, shuffle=True, random_state=42)  # Change n_splits to your desired number of folds

    for fold, (train_indices, val_indices) in enumerate(kf.split(common_bases), 1):
        ...
        train_image_dir_fold = train_image_dir / f'fold_{fold}'
        train_label_dir_fold = train_label_dir / f'fold_{fold}'
        val_image_dir_fold = test_image_dir / f'fold_{fold}'
        val_label_dir_fold = test_label_dir / f'fold_{fold}'

        train_image_dir_fold.mkdir(parents=True, exist_ok=True)
        train_label_dir_fold.mkdir(parents=True, exist_ok=True)
        val_image_dir_fold.mkdir(parents=True, exist_ok=True)
        val_label_dir_fold.mkdir(parents=True, exist_ok=True)

        # Copy data to the fold-specific directories
        for image, label in zip(train_images, train_labels):
            shutil.copy(image_dir / image, train_image_dir_fold / image)
            shutil.copy(label_dir / label, train_label_dir_fold / label)
        for image, label in zip(val_images, val_labels):
            shutil.copy(image_dir / image, val_image_dir_fold / image)
            shutil.copy(label_dir / label, val_label_dir_fold / label)

        # Load datasets for training and validation
        train_dataset = PVDataset(train_image_dir_fold, train_label_dir_fold, transform=transform)
        val_dataset = PVDataset(val_image_dir_fold, val_label_dir_fold, transform=transform)

        train_loader = DataLoader(train_dataset, batch_size=FLAGS.batch_size, shuffle=True, num_workers=2)
        val_loader = DataLoader(val_dataset, batch_size=FLAGS.batch_size, shuffle=False, num_workers=2)
        ...
        for epoch in range(1, FLAGS.num_epochs + 1):
            # Train
            train_loss, train_acc, train_f1_class, train_f1 = train(model, device, train_loader, optimizer, criterion, epoch, FLAGS.batch_size, file, class_weights)
            train_losses.append(train_loss)
            train_accs.append(train_acc)
            train_f1_scores.append(train_f1)
            ...
            if val_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    'val_accs': val_accs,
                    'train_f1_class_scores': train_f1_class,
                    'val_f1_class_scores': val_f1_class,
                    'train_f1_scores': train_f1_scores,
                    'val_f1_scores': val_f1_scores
                }, checkpoint_folder / f'model_fold_{fold}.pth')
            else:
                epochs_wo_improve += 1
                if epochs_wo_improve >= early_stopping_patience:
        ...
        plot_and_save_graphs(train_losses, val_losses, 'Loss', fold, checkpoint_folder)
        plot_and_save_graphs(train_accs, val_accs, 'Accuracy', fold, checkpoint_folder)
        plot_and_save_graphs(train_f1_scores, val_f1_scores, 'F1 Score', fold, checkpoint_folder)

        # After all epochs for this fold
        print(f"Completed Fold {fold}/{kf.n_splits}")

    # After all folds
    print("Cross-Validation Complete.")
    file.write("Cross-Validation Complete.\n")
